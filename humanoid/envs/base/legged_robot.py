...
class LeggedRobot(BaseTask):
    ...
    def step(self, actions):
        ...
        # print(self.base_euler_xyz)
        if self.common_step_counter > 1000:
            self.sim_params.gravity.y = 0.1
    ...
    def post_physics_step(self):
        ...
        self.feet_positions = self.rigid_state[:, self.feet_indices, 0:3]
        self.feet_velocities = self.rigid_state[:, self.feet_indices, 7:10]
        ...
        self.base_euler_xyz = get_euler_xyz_tensor(self.base_quat)
        # feet_quat may different for different robot
        self.feet_quat = self.rigid_state[:, self.feet_indices, 3:7]
        self.feet_euler_xyz = get_euler_xyz_tensor(self.feet_quat)

        feet_euler_xyz_tmp = self.feet_euler_xyz.clone()
        feet_euler_xyz_tmp[:, 0, 0] = 1.5708
        feet_euler_xyz_tmp[:, 0, 1] = 0
        feet_euler_xyz_tmp[:, 0, 2] = 1.5708
        feet_euler_xyz_tmp[:, 1, 0] = -1.5708
        feet_euler_xyz_tmp[:, 1, 1] = 0
        feet_euler_xyz_tmp[:, 1, 2] = -1.5708
        feet_norm_quat = quat_from_euler_xyz(feet_euler_xyz_tmp[:, :, 0], feet_euler_xyz_tmp[:, :, 1], feet_euler_xyz_tmp[:, :, 2])
        feet_norm_quat[:, :, 0] *= -1
        feet_norm_quat[:, :, 1] *= -1
        feet_norm_quat[:, :, 2] *= -1

        feet_quat_tmp = self.feet_quat.clone()
        feet_quat_tmp = quat_mul(feet_quat_tmp, feet_norm_quat)

        self.feet_euler_xyz = get_euler_xyz_tensor(feet_quat_tmp)
    ...
    def check_termination(self):
        ...
        roll_cutoff = torch.abs(self.base_euler_xyz[:, 0]) > 0.5
        pitch_cutoff = torch.abs(self.base_euler_xyz[:, 1]) > 0.5
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        # TODO not use ratio, direct assign ????
        # rand fix body com
        if self.cfg.domain_rand.randomize_com:
    ...
    # a function to update self.friction, not used
    ...
    # a function to update rigid shapes friction by self.friction, not used
    ...
    # a function to update fix body mass, not used
    ...
    def init_randomize_props(self):
        ...
        rotor_inertia = torch.tensor(self.cfg.asset.rotor_inertia, device=self.device, dtype=torch.float)
        self.joint_armatures_default = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        self.joint_armatures_default[:] = rotor_inertia
    ...
    def _get_heights(self, env_ids=None):
        ...
        if env_ids:
            # height_points is base frame
            # points is world frame
            points = quat_apply_yaw(self.base_quat[env_ids].repeat(1, self.num_height_points), self.height_points[env_ids]) + (self.root_states[env_ids, :3]).unsqueeze(1)
        else:
            points = quat_apply_yaw(self.base_quat.repeat(1, self.num_height_points), self.height_points) + (self.root_states[:, :3]).unsqueeze(1)

        # TODO points + border_size, because self.root_states[env_ids, :3] not including border???
    ...
    def _get_feet_heights(self, env_ids=None):
        if self.cfg.terrain.mesh_type == 'plane':
            return self.feet_positions[:, :, 2].clone()
        elif self.cfg.terrain.mesh_type == 'none':
            raise NameError("Can't measure height with terrain mesh type 'none'")

        if env_ids:
            points = self.feet_positions[env_ids].clone()
        else:
            points = self.feet_positions.clone()
        ...
        feet_height =  self.feet_positions[:, :, 2] - heights
        return feet_height