...
class DHOnPolicyRunner:
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            # obs_reshape = obs.view(self.env.num_envs, self.cfg.env.frame_stack, self.cfg.env.num_single_obs)
            # obs_reshape = obs.view(self.env.num_envs, 66, 47)
            # obs_std, obs_mean = torch.std_mean(obs_reshape, dim=(0,1))
            ...
            mean_value_loss, mean_surrogate_loss, mean_state_estimator_loss, mean_grad_penalty_loss, mean_sym_loss = self.alg.update()
            stop = time.time()
            learn_time = stop - start
            if self.log_dir is not None:
                self.log(locals())
            if it % self.save_interval == 0:
                self.save(os.path.join(self.log_dir, "model_{}_reward_{}.pt".format(it, self.reward_pt)))
            ep_infos.clear()
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar(
            "Loss/grad_penalty", locs["mean_grad_penalty_loss"], locs["it"]
        )
        self.writer.add_scalar(
            "Loss/_sym_loss", locs["mean_sym_loss"], locs["it"]
        )
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            self.reward_pt = statistics.mean(locs["rewbuffer"])
            self.writer.add_scalar(
        ...
        if len(locs["rewbuffer"]) > 0:
            log_string = (
                ...
                f"""{'sym loss:':>{pad}} {locs['mean_sym_loss']:.4f}\n"""
